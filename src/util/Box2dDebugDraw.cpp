...
namespace engine {
	sf::Color FromB2Color(b2Color c) {
		return sf::Color(
				clamp<uint8_t>(0, static_cast<uint8_t>(c.r * 255), 255),
				clamp<uint8_t>(0, static_cast<uint8_t>(c.g * 255), 255),
				clamp<uint8_t>(0, static_cast<uint8_t>(c.b * 255), 255));
	}
	...
	void Box2dDebugDraw::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color) {
		...
		auto window = m_scene->GetGame()->GetWindow();
		sf::Vector2f p(window->getView().getCenter().x - (window->getView().getSize().x / 2),
					   window->getView().getCenter().y - (window->getView().getSize().y / 2));
		for (int32 i = 0; i < vertexCount; i++) {
			s.setPoint(i, sf::Vector2f(m_scene->MeterToPixel(vertices[i].x) - p.x,
									   m_scene->MeterToPixel(vertices[i].y) - p.y));
		}
	...
	void Box2dDebugDraw::DrawCircle(const b2Vec2& center, float32 radius, const b2Color& color) {
		...
		sf::Vector2f p(window->getView().getCenter().x - (window->getView().getSize().x / 2),
					   window->getView().getCenter().y - (window->getView().getSize().y / 2));
		s.setPosition(m_scene->MeterToPixel(center.x) - m_scene->MeterToPixel(radius) - p.x,
					  m_scene->MeterToPixel(center.y) + m_scene->MeterToPixel(radius) - p.y);
		m_texture.draw(s);
	}

	void
	Box2dDebugDraw::DrawSolidCircle(const b2Vec2& center, float32 radius, const b2Vec2& axis, const b2Color& color) {
		sf::CircleShape s;
		s.setRadius(m_scene->MeterToPixel(radius));
